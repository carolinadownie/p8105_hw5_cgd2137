...
Making a boxplot of the number of viewers for each episode of each season.
...
#Problem 3--scraping Napolean Dyanmite reviews from Amazon and performing a sentiment analysis:
...
words_5_stars<-ratings_tidy_text %>% filter(stars == "5") 
words_5_stars<-table(words_5_stars$word) %>% data.frame() %>% arrange(desc(Freq)) %>% rename(words = Var1) %>% top_n(., 5)

words_1_star<-ratings_tidy_text %>% filter(stars == "1") 
words_1_star<-table(words_1_star$word) %>% data.frame() %>% arrange(desc(Freq)) %>% rename(words = Var1) %>% top_n(., 5)
